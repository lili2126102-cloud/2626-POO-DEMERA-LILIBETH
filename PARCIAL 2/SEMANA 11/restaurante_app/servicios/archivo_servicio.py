...
class ArchivoServicio:
    ...
    en formato JSON para productos, usuarios y ventas.
    ...
    def __init__(self, ruta_productos: str, ruta_usuarios: str, ruta_ventas: str) -> None:
        ...
        los archivos de persistencia de productos, usuarios y ventas.
        """
    ...
    def cargar_productos(self) -> List[Producto]:
        ...
        for idx, item in enumerate(datos):
            try:
                ...
                # Reconstruimos el producto (valida internamente que exista y sea correcto el stock)
    ...
    def guardar_ventas(self, ventas: List[Venta]) -> None:
        """
        Guarda la lista de objetos Venta en el archivo JSON.
        Crea los directorios necesarios si no existen.
        Lanza PermissionError en caso de no tener privilegios suficientes.
        """
        ...
        try:
            lista_dict = [v.to_dict() for v in ventas]
            with open(self.ruta_ventas, 'w', encoding='utf-8') as f:
                json.dump(lista_dict, f, indent=4, ensure_ascii=False)
        except PermissionError as e:
            raise PermissionError(f"No se tienen permisos de escritura sobre el archivo '{self.ruta_ventas}': {e}")
        except Exception as e:
            raise IOError(f"Error inesperado de E/S al escribir ventas: {e}")

    def cargar_ventas(self) -> List[Venta]:
        """
        Carga y reconstruye los objetos Venta almacenados en el archivo JSON.
        Lanza FileNotFoundError si el archivo no existe.
        Lanza JSONDecodeError si el formato no es JSON válido.
        Lanza PermissionError si no hay privilegios de lectura.
        Atrapa internamente KeyError y ValueError para omitir registros corruptos o inválidos.
        ...
        if not os.path.exists(self.ruta_ventas):
            raise FileNotFoundError(f"El archivo de ventas '{self.ruta_ventas}' no existe.")

        try:
            with open(self.ruta_ventas, 'r', encoding='utf-8') as f:
                datos = json.load(f)
        except json.JSONDecodeError as e:
            raise json.JSONDecodeError(f"Formato JSON inválido en '{self.ruta_ventas}': {e.msg}", e.doc, e.pos)
        except PermissionError as e:
            raise PermissionError(f"No se tienen permisos de lectura sobre el archivo '{self.ruta_ventas}': {e}")

        if not isinstance(datos, list):
            raise ValueError("La estructura del archivo JSON de ventas debe ser una lista.")

        ventas: List[Venta] = []
        for idx, item in enumerate(datos):
            try:
                if not isinstance(item, dict):
                    raise ValueError("Cada elemento de ventas debe ser un diccionario.")
                # Reconstruimos la venta
                venta = Venta.from_dict(item)
                ventas.append(venta)
            except KeyError as e:
                print(f"\n\033[93m[ADVERTENCIA] Venta #{idx+1} omitida en la carga: Clave faltante {e}.\033[0m")
            except ValueError as e:
                print(f"\n\033[93m[ADVERTENCIA] Venta #{idx+1} omitida en la carga: Datos inválidos. {e}.\033[0m")
